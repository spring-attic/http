...
Also you must override the default `spring.autoconfigure.exclude` property by setting it to void (e.g. `--spring.autoconfigure.exclude=`)
...
```
java -jar http_source.jar --spring.autoconfigure.exclude=
```
